package com.pingpetrus.whowroteit;
...
public class FetchBook extends AsyncTask<String, Void, String> {
    ...
    // Constructeur pour initialiser les WeakReference des TextView
    public FetchBook(TextView titleText, TextView authorText) {
        this.mTitleText = new WeakReference<>(titleText);
        this.mAuthorText = new WeakReference<>(authorText);
    }

    // Méthode exécutée en arrière-plan pour effectuer les opérations réseau
    @Override
    protected String doInBackground(String... strings) {
        return NetworkUtils.getBookInfo(strings[0]);
    }

    // Méthode exécutée après doInBackground() pour traiter le résultat
    @Override
